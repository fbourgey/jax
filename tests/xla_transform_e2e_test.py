...
"""End-to-end tests for custom XLA HLO transformations."""
...
class XlaTransformE2ETest(jtu.JaxTestCase):
  ...
  def setUp(self):
    super().setUp()
    # This test requires TPU for the custom scheduler to find async ops.
    # Collectives in shard_map on CPU might not generate async ops in the same way.
    if jax.devices()[0].platform != "tpu":
      self.skipTest("Skipping test for non-TPU devices")
