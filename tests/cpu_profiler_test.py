...
class CpuProfilerTest(jtu.JaxTestCase):
  ...
  def test_measure_multiple_iterations_aggregated(self):
    ...
    for i, t in enumerate(timings_agg):
      with self.subTest(i=i):
  ...
  def test_measure_warmup_exclusion(self):
    ...
    # The measurement should be small (<20ms) and should NOT include the compilation time.
  ...
  def test_measure_no_warmup(self):
    ...
    self.assertGreater(duration_first, duration_second * 2.0)
    self.assertGreater(duration_first - duration_second, 0.1)
