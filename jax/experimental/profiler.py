  ...
  for plane in profile_data.planes:
    ...
    for line in plane.lines:
      for event in line.events:
        ...
        if "_p" in stats:
          events_by_p[stats["_p"]] = event
        elif "_c" in stats:
          events_by_c[stats["_c"]] = event

  timings = []
  for context_id, prod_event in events_by_p.items():
    cons_event = events_by_c.get(context_id)
    if not cons_event:
      continue
    ...
    timings.append((prod_event.start_ns, hlo_op, duration_ms))

  # Sort by start_ns to guarantee chronological order across multiple threads.
  timings.sort(key=lambda x: x[0])

  return [(hlo_op, duration_ms) for _, hlo_op, duration_ms in timings]
